...
Any change to DD authentication or session dispatch requires the same full isolated live matrix under both an official email login state and an official mobile login state. Unit, contract, and mocked tests remain required, but neither they nor a one-flow login smoke can replace this matrix.

For each credential kind, read the live dependency graph for every current non-exploration seasonal build. Then cover plugin, configuration, and WA create, update, edit, readback, and delete, including file/media uploads where applicable and final cleanup. Use one task session per matrix run so all post-login operations exercise the shared path. Cleanup may remove only the isolated objects recorded as created by that run; do not modify pre-existing remote resources.

Verification evidence may contain only the safe credential kind, executed command, exit status, readback result, and cleanup result. Redact account names, credential values, passwords, tokens, Cookies, JWTs, signed URLs, `clientNo`, signatures, device proofs, raw WA strings, and raw configuration backup content. A failed or timed-out run must also verify that no DD sidecar, task broker, or broker state remains.
