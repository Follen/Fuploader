...
When validating a DD authentication change, mocks, contract tests, and a single login smoke are not sufficient. Run the same isolated live matrix once with an official email session and once with an official mobile session: read the dependency graph for every current non-exploration seasonal build, then create, update, edit, read back, and delete plugin, configuration, and WA resources, including their uploads and final cleanup. Evidence may record only the safe credential kind, executed commands, exit status, readback result, and cleanup result. It must redact account names, credential values, tokens, Cookies, JWTs, signed URLs, `clientNo`, raw WA strings, and raw backup content, and cleanup must touch only objects created by that test run.
