...
生产 CLI 的 NewBee API、认证、metadata、next API 和上传服务使用源码内固定的官方 HTTPS
origin 与路径前缀。`FUPLOAD_NEWBEE_API_BASE`、`FUPLOAD_NEWBEE_AUTH_BASE`、
`FUPLOAD_NEWBEE_NEXT_API_BASE`、`FUPLOAD_NEWBEE_METADATA_URL`、
`FUPLOAD_NEWBEE_UPLOAD_SERVER` 和 `FUPLOAD_NEWBEE_AUTH_DIR` 不得改变生产请求或凭据目录。
测试通过函数参数、adapter 或 mock 注入替代服务，不复用生产环境变量入口。

携带 access/refresh/resource/Creator token、device proof、Cookie、JWT、认证 code 或本地文件的
请求必须在发送前验证 scheme、host 和有效 port。只允许 HTTPS 和该请求声明的固定官方
origin；同 origin 重定向可以继续，任何跨 scheme、host 或 port 的重定向在重发 header/body
前失败。错误中只出现脱敏 origin 和拒绝原因。

NewBee 认证目录和 DD sidecar 状态目录由 Windows Known Folder API 得到的用户 Roaming
AppData 派生，不接受环境变量替换。目录链存在 reparse point 或最终目录逃出可信用户目录时
fail closed；凭据刷新继续使用同目录原子替换。

DD 候选 executable 在启动前必须通过 Windows Authenticode 链验证，并取得 signer 组织身份。
只有源码允许集合中的官方 NetEase 发布者可执行；使用发布者身份而非单一证书 thumbprint，
允许同一官方组织正常轮换证书。无签名、无效/撤销签名、无法解析发布者或未知发布者均排除。
