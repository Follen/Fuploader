...
- 使用当前受信官方 DD 客户端当前已持久化的账号完成全部非探索赛季 build 的插件和 WA 真实 create/update/edit/delete；配置只覆盖该账号实际返回可用云备份的 build。无可用云备份的 build 必须在证据中明确标记为不适用，不能静默跳过。邮箱和手机登录态的自动分流均由自动化合同测试覆盖，不要求为验收切换当前账号。
...
- A6：官方客户端当前已持久化账号下，当前全部非探索赛季 build 的依赖读取、插件和 WA 的真实 create/update/edit/delete、上传、读回和最终清理全部成功；配置在该账号实际返回可用云备份的每个 build 上完成同样的全 CRUD，并为无可用云备份的 build 记录明确的不适用原因。
- A7：当前账号的真实证据记录所用安全 credential kind、实际命令、退出状态、读回、不适用项、最终实现 commit、DD 版本、资源哈希与清理结果，不记录敏感凭据；邮箱和手机分流矩阵均由自动化测试通过，不要求切换当前持久化账号重复真实矩阵。
- A8：DD Skill/reference 和完整 `dd-publishing` Spec 明确说明自动邮箱/手机分流、支持矩阵、失败语义、共用登录后代码，以及仅对当前已持久化账号执行真实验证的边界。
...
- 真实全量资源矩阵只对当前已持久化账号执行；邮箱和手机登录态的分流、参数与共用登录后管线均由自动化合同测试覆盖，不为验收要求用户切换账号。
...
- 真实检查在当前已持久化账号下执行 `session doctor/start/status/stop`、JWT/作者 API ready、全部非探索赛季 build 的读取矩阵、插件与 WA 的全 build create/update/edit/delete，以及该账号每个可用云备份 build 的配置 create/update/edit/delete、二进制上传、写后读回和依赖顺序清理。
- 真实证据必须绑定最终实现 commit 和当前 DD 版本/资源哈希，输出只含安全枚举、摘要、长度、SHA-256、业务码与对象测试标识；邮箱和手机登录态分流另由自动化矩阵完整覆盖。
