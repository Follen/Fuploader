...
- `need_anchor_vip -> vip_levels`；关闭时清空等级。
...
三类资源的公共字段为 `scope`、`share_code_life_type`、`need_buy`、`price_fen`、`buy_life_type`、`jump_room`、`room_id`、`channel_id`、`channel_type`、`sync_room`、`creation_statement`、`with_associate`、`associated_acts`、`need_anchor_vip`、`vip_levels`。金额输入为分，UI 可选范围 10..20000 分。

公共条件规则必须与官方 builder 一致：private 关闭 anchor VIP 和同步；未关联房间清空 room/channel 并关闭同步；未关联内容清空 `associated_acts`；免费价格为零；需要购买时校验价格和购买有效期；plugin 与 WA public 强制 `share_code_life_type=forever`；config public 省略该字段，config private 才提交它。
...
每次 create/update/edit submit 都对 `!WA:2!` 内容调用官方原生 bridge 重新解析，即使字符串未改变；解析结果不匹配或缺失时停止。非 WA2 清空 parse 字段。`with_file=true` 要求材质 ZIP 和安装路径；`with_file=false` 按官方 builder 保留现有 `file_path/file_install_path`，不擅自清空。

create 允许全部首次字段；outer usage mode 为 create-only。update 只允许 content、严格递增 version、update_desc、材质及官方更新页字段。edit 允许名称、版本归属、分类、说明、媒体和允许的公共设置。两者先 GET `/wa/detail` 和动态选项，再构建完整 `/wa/modify` payload。
...
错误保留 resource、operation、脱敏 endpoint、HTTP status、业务 code 和精简消息；native sidecar 失败还保留有界异常消息和 `code`/`error_code`。签名 URL query 内的 credential、signature 和 token 必须在 sidecar 返回前脱敏；不输出 token、Cookie、JWT、登录 code、clientNo、signed URL、原始 WA、raw backup 或完整 payload。写成功仅表示业务 code 接受，不等于审核通过。每次 create/update/edit/delete 后必须调用对应 detail/list/version 读回；插件 update 以作者列表 `latest_version` 为成功判据，详情和版本历史只作补充。两份官方投影都未反映提交字段时才是不确定结果，先 GET，不自动重发。

## 验证

合同测试覆盖全部 endpoint 参数、动作 allowlist、字段锁、父子依赖、上游切换、selector provenance、上传 descriptor、特殊文件名、错误阶段、脱敏和 readback。进程测试验证 GUI 冲突时零 relogin、一次 task 一次 relogin、并发串行、显式 logout、失败即停和空闲退出。
