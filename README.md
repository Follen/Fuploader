...
该 Skill 不会因普通提及“发布”“新手盒子”或“DD”而自动触发。Agent 会先询问平台、资源和动作，读取所需契约与动态选项，生成 JSON 输入并执行 dry-run；只有在展示完整写入计划并得到确认后才会执行真实写入。
...
4. 先执行 `--dry-run`，再向用户展示所有修改、保留和清空字段。
5. 得到明确确认后串行写入，每步成功后立即读回验证。
6. 遇到 `verification_required` 时先读取远端状态，再决定是否重试。
